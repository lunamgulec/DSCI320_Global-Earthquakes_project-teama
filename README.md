...
Our chosen dataset includes information about earthquakes, where each record represents an individual earthquake and its detailed attributes, such as location (`latitude`, `longitude`, `locationSource`, etc.), time of occurrence (`time`), magnitude (`mag`), depth (`depth`), event type (`type`), and more (detailed below). The dataset is publicly available and can be accessed through the [USGS Earthquake Hazards Program](https://earthquake.usgs.gov/earthquakes/feed/v1.0/csv.php), which provides live earthquake feeds that update every minute, and can be filtered/chosen per time range (past hour, week, or month). 
